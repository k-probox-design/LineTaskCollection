...
def test_discover_maps_via_glob_when_outside_sessions(tmp_path, monkeypatch):
    # git clone を /tmp 等ネイティブ fs で実行する場合、self_path が /sessions 配下にないので
    # session_base=None。その場合でも /sessions/*/mnt/<name> の glob フォールバックでマウントを
    # 解決できること（git clone 経路の土台）。
    ...
    maps = mounts.discover_maps(WIN_ATSIGN, self_path="/tmp/linetask/pc_worker/app/mounts.py")
    assert maps == [(str(real), WIN_ATSIGN)]
